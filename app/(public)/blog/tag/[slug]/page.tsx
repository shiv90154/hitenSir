...
import { listByTagSlug } from "@/lib/db/blogs";
...
export default async function BlogTagPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const posts = await listByTagSlug("ARTICLE", slug);

  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-16">
      <p className="text-xs uppercase tracking-wide text-ink-soft">Tag</p>
      <h1 className="font-display text-4xl font-semibold text-ink">#{tag.name}</h1>
